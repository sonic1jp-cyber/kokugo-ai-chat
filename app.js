...
function chooseModel(m){
  // 2回目以降の応答（ユーザーが情報を提供した後）はOpusで深く答える
  if(messages.length>=2)return MODEL_DEEP;
