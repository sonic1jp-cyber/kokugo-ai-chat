...
var apiKey='',messages=[],usageCount=0,isLoading=false,lowUsageWarned=false,userProfile={};
...
window.addEventListener('DOMContentLoaded',function(){
  ...
  if(!apiKey)setTimeout(function(){document.getElementById('settings-modal').classList.add('active')},500);
});

function loadState(){
  try{
    apiKey=ss_get('airns_key')||'';
...
function saveState(){
  try{ss_set('airns_key',apiKey);ss_set('airns_usage',usageCount.toString());ss_set('airns_messages',JSON.stringify(messages))}catch(e){}
}
...
async function handleSubmit(e){
  ...
  if(!apiKey){document.getElementById('settings-modal').classList.add('active');return}
...
function openSettings(){document.getElementById('api-key-input').value=apiKey?'••••••••':'';document.getElementById('settings-modal').classList.add('active')}
function closeSettings(){document.getElementById('settings-modal').classList.remove('active')}
function saveSettings(){var k=document.getElementById('api-key-input').value.trim();if(k&&k!=='••••••••'){apiKey=k;saveState()}closeSettings()}
